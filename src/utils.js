export var highlightedDates = {
  "March 2024": [
    { date: "2024-03-01", amount: 5000 },
    { date: "2024-03-05", amount: 2800 },
    { date: "2024-03-10", amount: 4200 },
    { date: "2024-03-15", amount: 3700 },
    { date: "2024-03-20", amount: 5500 },
    { date: "2024-03-25", amount: 4900 }
  ],
  "April 2024": [
...
export function findLowestAmountDates(highlightedDates) {
...
export function generateMonthData(startMonth, numMonths, highlightedDatesMap) {
  const months = [];
  ...
  for (let i = 0; i < numMonths; i++) {
    ...
    const firstDayOfMonth = currentDate.getDay(); // Get the day of the week for the first day of the month
    ...
    for (let j = 0; j < firstDayOfMonth; j++) {
      monthData.days.push({ date: null, key: `empty-${j}` });
    }
    ...
    for (let j = 1; j <= lastDayOfMonth.getDate(); j++) {
      ...
      const dayOfMonth = String(j).padStart(2, '0'); 
      const dayISO = `${year}-${month}-${dayOfMonth}`;
      const isHighlighted = highlightedDatesMap[monthData.month] && highlightedDatesMap[monthData.month].has(dayISO);
      const isLowest = isSameDay(new Date(lowestAmountDateForMonth), day);
      monthData.days.push({
        date: dayISO,
        key: dayISO,
        dayOfWeek: weekdays[(firstDayOfMonth + j - 1) % 7],
